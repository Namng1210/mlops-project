...
def custom_predict(y_prob, threshold, index):
    """Custom predict function that defaults to an index if condition are not met"""
...
def predict(texts, artifacts):
    ...
    predictions = [{"input_text": texts[i], "predicted_tags": tags[i]} for i in range(len(tags))]
    return predictions
